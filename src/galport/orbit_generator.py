...
class OrbitGenerator():
    ...
    A class for generating initial conditions for various orbit families in
    barred galactic potentials.

    
    Parameters
    ----------
    potential : agama.Potential
        Galactic potential model (can be non-axisymmetric, e.g., with bar).
    Omega : float, optional
        Pattern speed of the non-axisymmetric perturbation.
        Default: 0.0
    ...
    * ``ban_down`` — banana orbits with tips pointing downward (-z)
    ...
    find_bar_boundaries(x0=0.1)
        Find bar boundaries by locating L1 Lagrange points.
        The maximum HJ corresponds to the L1 point.
    """
